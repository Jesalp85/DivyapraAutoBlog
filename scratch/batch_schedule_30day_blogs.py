...
# 7 Core Store Products & CDN Images Registry
PRODUCTS = [
    {
...
# 5 Core Categories Taxonomy (100% compliant)
CATEGORIES = ["Heritage Recipes", "Health & Spices", "Buying Guides", "Pickle Pairings", "Artisanal Craft"]

# Topics Database for 30 Days (150 topics across 30 days, 5 posts per day)
TOPIC_TEMPLATES = [
...
def clean_title(title):
    # Ensure title tag length is strictly <= 60 characters
    if len(title) > 59:
        # Trim at last word boundary under 58 chars
        trimmed = title[:58].rsplit(' ', 1)[0]
        return trimmed
    return title

def generate_high_depth_html(title, keyword, prod_idx, category):
    p = PRODUCTS[prod_idx]
...
def run_schedule_and_publish():
    start_date = datetime.date(2026, 8, 11)  # Today is August 11, 2026
    posting_times = ["09:00:00+05:30", "12:00:00+05:30", "15:00:00+05:30", "18:00:00+05:30", "21:00:00+05:30"]

    print("🚀 STARTING 30-DAY HIGH-DEPTH BATCH PUBLISHING & SCHEDULING TO SHOPIFY...")
    print("Store: divyaprabhafoods.com | Workspace: /DivyaPrabha Food\n")

    published_today_count = 0
    scheduled_future_count = 0
    error_count = 0

    total_topics = min(len(TOPIC_TEMPLATES), 150) # 150 topics (30 days x 5 posts/day)
    ...
    for idx in range(total_topics):
        ...
        html_body = generate_high_depth_html(title, keyword, prod_idx, category)
        clean_t = clean_title(title)
        
        # Calculate word count for audit check
        text_words = len(re.sub(r'<.*?>', '', html_body).split())
        ...
        # If date is today (Aug 11), publish immediately live (published=True)
        # If date is future (Aug 12+), schedule post (published=False, published_at=timestamp)
        is_today = (sched_date == start_date)
        
        payload = {
            "article": {
                "title": clean_t,
                "author": "Baa & The DivyaPrabha Culinary Team",
                "tags": category,  # Strict single tag from 5 core categories
                "summary_html": excerpt,
                "body_html": html_body,
                "published": is_today,
                "published_at": sched_timestamp if not is_today else f"{sched_date.isoformat()}T{posting_times[time_idx]}"
            }
        }
        ...
        try:
            with urllib.request.urlopen(req, context=ctx) as resp:
                ...
                if is_today:
                    ...
                    print(f"✅ [TODAY LIVE {published_today_count}/5] Words: {text_words} | Tag: '{category}' | Title ({len(clean_t)} chars): '{clean_t}' (ID: {art.get('id')})")
                else:
                    scheduled_future_count += 1
                    if scheduled_future_count <= 10 or scheduled_future_count % 10 == 0:
                        print(f"📅 [SCHEDULED #{scheduled_future_count}] {sched_timestamp} | Words: {text_words} | Tag: '{category}' | Title: '{clean_t}'")
        except Exception as e:
            error_count += 1
            print(f"❌ Error for '{clean_t}':", e)

    print("\n========================================================")
    print("🎉 30-DAY HIGH-DEPTH BATCH PUBLISHING & SCHEDULING COMPLETE!")
    print(f"✅ Published LIVE Today (Aug 11): {published_today_count} Articles (100% Rule Compliant)")
    print(f"📅 Scheduled for Future (Aug 12 - Sept 9): {scheduled_future_count} Articles")
    print(f"❌ Errors: {error_count}")
    print("========================================================")

if __name__ == "__main__":
    run_schedule_and_publish()
